  ...
  useFrame(({ clock }) => {
    ...
    for (let h = 0; h < size; h++) {
      for (let w = 0; w < size; w++) {
        let verticesIndex = (h * size + w) * 3 + 2; // +2 for the z-coordinate

        // let verticesIndex = h * size + w;
        const timeFactor = clock.elapsedTime + (h + w) * waveFrequency;
        // Smoothly varying wave patterns
        const hWave =
          waveAmplitude * Math.sin(h * waveFrequency + clock.elapsedTime);
        const wWave =
          waveAmplitude * Math.cos(w * waveFrequency + clock.elapsedTime);

        // Offset based on combined wave patterns
        const z = hWave + wWave;
